...
def negative_sample(train_dict, all_movie_list, shrink):
    '''
    ...
    #print(all_movie_size)       #1675
    all_user_negetive = {}
    for user in train_dict:     #构建每个用户的负样本
        ...
        while len(user_negative_movie) < negative_size:
            ...
            if negative_movie not in user_train_movie and negative_movie not in user_negative_movie:
                print(negative_movie)

                user_negative_movie.append(negative_movie)
                # line = str(user) + '\t' + str(negative_movie) + '\n'
                # fw_negative.write(line)
        if user not in all_user_negetive:
            all_user_negetive.update({user:user_negative_movie})

    return all_user_negetive
        
if __name__ == '__main__':
    
    parser = argparse.ArgumentParser(description=''' Sample Negative Movies for Each User''')

    parser.add_argument('--train', type=str, dest='train_file', default='data/ml/train_data.txt')
    ...
    fr_train = pd.read_table(train_file,names=["userid", "itemid", "rating", "timestamp",
